...
function CreateUserModal({ onSave, onClose }: { onSave: (user: PublicUser) => void; onClose: () => void }) {
  const form = useForm<CreateUserValues>({
    defaultValues: { firstName: "", lastName: "", email: "", phone: "", password: "" },
    mode: "onChange",
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    resolver: joiResolver(createUserSchema) as any,
  });
  ...
  const handleSubmit = form.handleSubmit(async (data) => {
    const res = await fetch("/api/admin/users", {
      ...
      body: JSON.stringify({ ...data, isAdmin: isAdminChecked ? 1 : 0 }),
    });
    if (res.ok) onSave(await res.json());
    else { const { error } = await res.json().catch(() => ({ error: "Unknown error" })); alert(error ?? "Failed to create user."); }
  });
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ background: "white", borderRadius: 12, padding: 28, width: "100%", maxWidth: 480, boxShadow: "0 8px 32px rgba(0,0,0,0.2)" }}>
        ...
        <FormProvider {...form}>
          <FormGrid cols={2}>
            ...
            <Field<CreateUserValues> name="password" label="Password" type="password" placeholder="Min 6 chars" rules={{ required: true }} />
          </FormGrid>
        </FormProvider>
        <label style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 16, fontSize: 14, cursor: "pointer", color: "#222" }}>
          <input type="checkbox" checked={isAdminChecked} onChange={(e) => setIsAdminChecked(e.target.checked)} /> Admin
        </label>
        <div style={{ display: "flex", gap: 10, marginTop: 24, justifyContent: "flex-end" }}>
          <Button variant="secondary" onClick={onClose}>Cancel</Button>
