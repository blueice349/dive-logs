...
export default function CertsTab({ user }: { user: PublicUser }) {
  ...
  return (
    <div>
              ...
              <Card>
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <h3 style={{ margin: "0 0 6px", color: "#1565c0", fontWeight: 700, fontSize: 17 }}>
                      {cert.certName}
                    </h3>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "4px 16px", fontSize: 14, color: "#555" }}>
                      {cert.agency && (
                        <span>
                          <strong style={{ color: "#444" }}>Agency:</strong> {cert.agency}
                        </span>
                      )}
                      {cert.certDate && (
                        <span>
                          <strong style={{ color: "#444" }}>Date:</strong> {formatDate(cert.certDate)}
                        </span>
                      )}
                      {cert.certNumber && (
                        <span>
                          <strong style={{ color: "#444" }}>Cert #:</strong> {cert.certNumber}
                        </span>
                      )}
                    </div>
                    {cert.notes && (
      ...
      {showAdd && (
        <CertModal cert={null} onSave={handleSaved} onClose={() => setShowAdd(false)} />
      )}
      {editingCert && (
        <CertModal cert={editingCert} onSave={handleSaved} onClose={() => setEditingCert(null)} />
      )}
      {deletingCert && (
        <ConfirmModal
